...
| 프로젝트                                                      | 내용                                                                                                                                                                         | 주요 코드                                                                                        |
| --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| [FRUS · 웹 크롤링](Cursor/frus_crawl.py)                      | **데이터:** 미국 국무부 공개 외교사료 *Foreign Relations of the United States (FRUS)* — 행정부별로 **볼륨 → 목차(섹션) → 문서 번호 구간**을 따라 문서 단위 수집. 페이지 요청이 많고 HTML 파싱 부담이 커 **FRUS XML** 파이프라인으로 전환. | **Cursor:** 하나의 파이프라인으로 정리.                                         |
| [FRUS · XML → CSV](Cursor/frus_xml_to_csv.py)             | **데이터:** [HistoryAtState/frus `volumes/*.xml](https://github.com/HistoryAtState/frus/tree/master/volumes)` TEI를 **CSV로 구조화** (웹 전체 크롤 대신 Git 원문 사용).                       | **Cursor:** GitHub tree로 볼륨 목록, raw XML 파싱 |
| [Author affiliation](Cursor/author_affiliation_enrich.py) | **데이터:** 논문 표의 **저자**에 대해 **소속·기관** 등 메타 보강.                | **Cursor:** (Scholar 검색 -> **OpenAlex API** 사용).           |
| [News](Cursor/news_unibook_metadata.py)                   | **데이터:** 유니북 **URL**에서 서지 6필드(주기사명·서명·권호·발행처·발행일자·페이지) **한 행 CSV**.                                                                                                        | **Cursor:** 시간 단축 및 코드 간편화                                  |
