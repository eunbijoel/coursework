...
| 기능              | 설명                                                                                        |
| --------------- | ----------------------------------------------------------------------------------------- |
| **로컬 실행·모델 연동** | Mac, Windows, Linux에서 내 컴퓨터에 설치해 동작. OpenAI, Anthropic(Claude), 로컬 모델 등을 선택해 연결 |
| **채팅 인터페이스**    | WhatsApp, Telegram, Discord, Slack, Signal, iMessage 등에서 대화로 명령                       |
| **장기 기억(메모리)**  | 세션·사용자 선호·맥락을 유지해 개인화                                                          |
| **브라우저 제어**     | 웹 검색, 폼 작성, 데이터 추출 등(문서의 Browser Control 등)                                              |
| **시스템 접근**      | 파일 읽기/쓰기, 셸 명령 실행 등(문서의 Full System Access / 샌드박스 선택 등 안내)                               |
| **스킬(Skills)**  | 커뮤니티 스킬을 쓰거나 직접 만들어 확장                                                             |
...
> ```
>
> 설치 후 재부팅하면 Ubuntu가 기본으로 설치 됨. 이후 모든 명령어는 **WSL2 터미널 안에서** 실행
>
...
- WhatsApp 현재 인증 파일명 등은 [WhatsApp 채널 문서](https://docs.openclaw.ai/channels/whatsapp) 참고
---
...
- **WhatsApp 전용 번호 권장**: 기존 개인 번호보다 전용 번호 사용
- **VoIP 번호 비권장**: WhatsApp 정책상 VoIP 번호는 차단될 가능성 높음
- **페어링 승인**: 처음 연결 시 반드시 페어링을 승인해야 하며, 알 수 없는 요청은 승인 금지
- **허용 목록 설정**: 구성 파일에서 특정 번호를 자동 허용하도록 허용 목록을 설정
- **데몬 관리**: 데몬이 설치된 경우 시스템 리소스(CPU, 메모리)를 지속적으로 사용; 필요 없을 시 중지
- **모델 비용**: API 키 방식 사용 시 AI 모델 호출 비용 발생; 주기적으로 사용량 확인
